...
public class OrderService {
    ...
    // @Transactional은 수해된 메서드 수행도중 예외가 발생하면 이미 수행되었던
    // 동작을 모두 롤백(rollback=원상복귀)시키도록 명령하는 어노테이션
    // 주문세부의 특정 제품의 재고가 부족해서 예외가 발생하면 전체 주문, 주문세부의
    // 저장되었던 내용들은 모구 취소되고 롤백됨!!
        ...
        for (OrderDetail data : orderRequestDTO.getOrderDetails()) {
            ...
            if (product.getStock() - data.getOrderQuantity()<0) {
                throw new InvalidRequestException(
                        "요청하신 주문수량이 재고를 초과합니다." + product.getProductNum() + "현재재고: " + product.getStock());
